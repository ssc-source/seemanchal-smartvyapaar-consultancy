...
export async function generateMetadata() {
  try {
    return await Promise.race([
      generatePageMetadata('services', fallbackMetadata),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('Metadata generation timeout')), 3000)
      )
    ]);
  } catch (error) {
    console.warn('Services metadata generation failed:', error.message);
    return fallbackMetadata;
  }
}

export default async function ServicesPage() {
  const services = await contentAdapter.resolveServices().catch(() => []);
