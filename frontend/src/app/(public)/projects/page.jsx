...
export async function generateMetadata() {
  try {
    return await Promise.race([
      generatePageMetadata('projects', fallbackMetadata),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('Metadata generation timeout')), 3000)
      )
    ]);
  } catch (error) {
    console.warn('Projects metadata generation failed:', error.message);
    return fallbackMetadata;
  }
}

export default async function ProjectsPage() {
  const projects = await contentAdapter.resolveProjects().catch(() => []);
