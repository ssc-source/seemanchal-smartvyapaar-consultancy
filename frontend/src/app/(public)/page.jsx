...
import FloatingPromoCards from "@/components/home/floating-promo-cards";
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('Metadata generation timeout')), 1500)
      )
    ]);
  } catch (error) {
    console.warn('Metadata generation failed, using defaults:', error.message);
    return {
      title: 'Seemanchal SmartVyapaar Consultancy',
...
export default async function Home() {
  // Fetch all data in parallel for faster load time (max 5 seconds total)
  const [pageData, services, projects, testimonials] = await Promise.all([
    contentAdapter.resolveHomepage().catch(() => ({
      hero: { headline: 'Welcome', subheadline: 'Loading...', primaryCTA: { label: 'Get Started', href: '#' }, secondaryCTA: { label: 'Learn More', href: '#' } },
      stats: [],
      useCases: [],
      whyChooseUs: [],
    ...
    contentAdapter.resolveServices().catch(() => []),
    contentAdapter.resolveProjects().catch(() => []),
    contentAdapter.resolveTestimonials().catch(() => []),
  ]);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between">
      ...
      <section className="w-full relative bg-linear-to-b from-slate-50 via-white to-white pt-28 pb-20 px-6 lg:px-8 flex flex-col items-center text-center overflow-hidden">
        ...
        <div className="absolute top-0 inset-x-0 h-100 bg-linear-to-b from-brand-accent/5 to-transparent pointer-events-none" />
        ...
        {/* Floating/Inline visual cards for Tech Internship & Future Skills Lab */}
        <FloatingPromoCards />
      </section>
      ...
      {/* ─── Future Skills Lab Section ─── */}
      <section className="w-full bg-slate-50/60 py-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12 space-y-4">
            <p className="text-sm font-semibold uppercase tracking-[0.28em] text-brand-accent">Future Skills Lab</p>
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900">Future-ready learning for schools and students</h2>
            <p className="text-slate-500 max-w-3xl mx-auto text-lg">Industry-aligned programs, hands-on workshops, and career discovery pathways designed to help schools build future skills in AI, digital citizenship, entrepreneurship, and career readiness.</p>
          </div>

          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4 mb-12">
            <div className="rounded-3xl bg-white border border-slate-200 p-8 shadow-sm hover:shadow-lg transition-shadow">
              <h3 className="text-xl font-semibold text-slate-900 mb-3">AI Literacy</h3>
              <p className="text-slate-500 leading-relaxed">Equip students with practical AI understanding, ethical use, and real-world problem solving.</p>
            </div>
            <div className="rounded-3xl bg-white border border-slate-200 p-8 shadow-sm hover:shadow-lg transition-shadow">
              <h3 className="text-xl font-semibold text-slate-900 mb-3">Digital Citizenship</h3>
              <p className="text-slate-500 leading-relaxed">Teach safe, responsible, and empowered digital behavior across online learning environments.</p>
            </div>
            <div className="rounded-3xl bg-white border border-slate-200 p-8 shadow-sm hover:shadow-lg transition-shadow">
              <h3 className="text-xl font-semibold text-slate-900 mb-3">Innovation & Entrepreneurship</h3>
              <p className="text-slate-500 leading-relaxed">Develop creativity, design thinking, and project-based startup skills for the next generation.</p>
            </div>
            <div className="rounded-3xl bg-white border border-slate-200 p-8 shadow-sm hover:shadow-lg transition-shadow">
              <h3 className="text-xl font-semibold text-slate-900 mb-3">Career Discovery</h3>
              <p className="text-slate-500 leading-relaxed">Help students explore future career paths and gain clarity on education-to-work transitions.</p>
            </div>
          </div>

          <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            <CTA label="Explore Future Skills" href="/future-skills" size="lg" />
            <CTA label="Partner with Schools" href="/future-skills/become-partner" variant="outline" size="lg" />
          </div>
        </div>
      </section>
